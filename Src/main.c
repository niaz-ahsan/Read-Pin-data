...
int main(void)
{
	...
	uint32_t volatile *const RCC_AHB1 = (uint32_t*) 0x40023830;
	*RCC_AHB1 |= (1 << 0); // enabling 0th bit for GPIO A

	// GPIO A port mode register address
	uint32_t volatile *const GPIOA_port_mode_reg = (uint32_t*) 0x40020000;
	...
	*GPIOA_port_mode_reg &= ~(3 << 0);
	...
	uint32_t const volatile *const GPIOA_input_data_reg = (uint32_t*) 0x40020010;
